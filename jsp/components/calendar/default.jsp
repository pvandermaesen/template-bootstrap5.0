		...
		<tbody>
			<c:forEach var = "w" begin = "0" end = "5">
				...
				<c:forEach var = "d" begin = "0" end = "6">
					<c:set var="icals" values="calendar.monthEvents[calendar.daysBloc[w][d].monthDay]" />
					<td class="${calendar.daysBloc[w][d].active?'active':'unactive'} ${calendar.daysBloc[w][d].toDay?'today':'nottoday'} ${empty icals || fn:length(icals)==0?'empty':'notempty'}">
						...
						<ul class="events">
							<c:forEach var="ical" items="${icals}">
							<li title="${ical.summary}" class="cal-${ical.categories}">								
								...
								<div class="summary">${ical.summary}</div>								
							</li>
							</c:forEach>
						</ul></c:if>
					</td>
