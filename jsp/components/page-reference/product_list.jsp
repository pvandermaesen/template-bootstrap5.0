		...
		<c:forEach items="${pages}" var="page" varStatus="status">
			...
			<li class="item item-${status.index+1} ${status.index%2==0?'odd':'even'} ${page.toTheTopLevel>0?'top':''}${cssClass}" ${style}>
				...
				<div class="caption">
					...
					<h3><a ${popup?'class="popup" ':''}title="${page.titleForAttribute}" href="${page.linkOn}" ${globalContext.openExternalLinkAsPopup && !page.linkRealContent?'target="_blank"':''}>${page.title}</a></h3>
